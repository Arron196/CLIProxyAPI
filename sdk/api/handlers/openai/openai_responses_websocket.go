...
func (h *OpenAIResponsesAPIHandler) ResponsesWebsocket(c *gin.Context) {
	...
	for {
		...
		nextSessionRequestSnapshot = repairResponsesWebsocketToolCalls(toolPairState, nextSessionRequestSnapshot)
		...
		completedOutput, terminalStatus, errForward := h.forwardResponsesWebsocket(c, conn, cliCancel, dataChan, errChan, &wsBodyLog, passthroughSessionID, toolPairState)
		if errForward != nil {
...
func normalizeResponseSubsequentRequest(rawJSON []byte, lastRequest []byte, lastResponseOutput []byte, allowIncrementalInputWithPreviousResponseID bool) ([]byte, []byte, *interfaces.ErrorMessage) {
	...
	// When a client replaces local websocket history after compaction, the next
	// response.create may already contain historical assistant output items.
	// Treat that as a transcript reset instead of incrementally appending stale state.
